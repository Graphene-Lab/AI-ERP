...
{
	[PageComponent(Label = "Field URL", Library = "AI", Description = "This field will validate local and static URLs. Will present them accordingly", Version = "0.0.1", IconClass = "fas fa-globe-africa")]
	public class PcFieldUrl : PcFieldBase
	{
		protected ErpRequestContext ErpRequestContext { get; set; }

		public PcFieldUrl([FromServices]ErpRequestContext coreReqCtx)
		{
			ErpRequestContext = coreReqCtx;
		}

		public class PcFieldUrlOptions : PcFieldBaseOptions
		{
		...
		public async Task<IViewComponentResult> InvokeAsync(PageComponentContext context)
		{
			...
			try
			{
				...
				var options = PcFieldUrlOptions.CopyFromBaseOptions(baseOptions);
				if (context.Options != null)
				{
					options = JsonConvert.DeserializeObject<PcFieldUrlOptions>(context.Options.ToString());
