		...
		public async Task<IViewComponentResult> InvokeAsync(BaseErpPageModel pageModel)
		{
			...
			#region === <script> ===
			{
				...
				#region << site.js >>
				{
					...
					{
						Src = "/_content/AI.Erp.Web/js/site.js?cb=" + cacheKey
					});
				}
				#endregion
				...
				#region << js-cookie >>
				{
					if (!includedScriptTags.Any(x => x.Src.Contains("/js-cookie")))
					{
						scriptTagsToInclude.Add(new ScriptTagInclude()
						{
							Src = "/_content/AI.Erp.Web/lib/js-cookie/js.cookie.min.js?cb=" + cacheKey
						});
					}
				}
				#endregion
				...
				foreach (var componentName in stencilComponents)
				{
					scriptTagsToInclude.Add(new ScriptTagInclude()
					{
						Src = $"/_content/AI.Erp.Web/js/{componentName}/{componentName}.esm.js",
						Type = "module"
					});

					scriptTagsToInclude.Add(new ScriptTagInclude()
					{
						Src = $"/_content/AI.Erp.Web/js/{componentName}/{componentName}.js",
						IsNomodule = true
					});
				}
