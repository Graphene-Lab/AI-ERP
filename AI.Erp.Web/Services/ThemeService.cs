...
namespace AI.Erp.Web.Services
{
	public class ThemeService : BaseService
	{
		...
		public string GenerateStyleFrameworkContent()
		{
			...
			string file = FileService.GetEmbeddedTextResource("bootstrap.css", "AI.Erp.Web.Theme");
		...
		public string GenerateStylesContent()
		{
			var cssContent = "";
			var buildDir = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
			string file = FileService.GetEmbeddedTextResource("styles.css", "AI.Erp.Web.Theme");
			cssContent = ApplyThemeSettingsToString(file);
			return cssContent;
		}
