	...
	public partial class MailPlugin : ErpPlugin
	{
		private static void Patch20190419(EntityManager entMan, EntityRelationManager relMan, RecordManager recMan)
		{
			...
			#region << ***Update sitemap node*** Sitemap node name: page >>
			{
				...
				new AI.Erp.Web.Services.AppService().UpdateAreaNode(id, areaId, name, label, labelTranslations, iconClass, url, type, entityId, weight, access, entityListPages, entityCreatePages, entityDetailsPages, entityManagePages, AI.Erp.Database.DbContext.Current.Transaction);
			}
			#endregion
