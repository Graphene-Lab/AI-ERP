	...
	public partial class SdkPlugin : ErpPlugin
	{
		private static void Patch20190227(EntityManager entMan, EntityRelationManager relMan, RecordManager recMan)
		{
			...
			#region << ***Update sitemap node*** Sitemap node name: page >>
			{
				...
				new AI.Erp.Web.Services.AppService().UpdateAreaNode(id, areaId, name, label, labelTranslations, iconClass, url, type, entityId, weight, access, new List<Guid>(), new List<Guid>(), new List<Guid>(), new List<Guid>(), AI.Erp.Database.DbContext.Current.Transaction);
			}
			#endregion
